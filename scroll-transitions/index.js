  ...
  if (rainbow) {
    attributes.push({
      ...
      data: (i, total) => {
        ...
        color.setHSL((i / total) * 2, 0.6, 0.7);
...
const ambientLight = new THREE.AmbientLight("#fffacc", 0.1);
scene.add(ambientLight);

const light = new THREE.SpotLight(0xfffacc, 1, 80, Math.PI * 0.25, 1, 2);
...
const instances = [
  createInstance({
    ...
    points: [
      ...
      () => getArrayWithNoise([0, -10, 0], 8),
      () => getArrayWithNoise([0, 10, 0], 2),
  ...
  createInstance({
    ...
    duration: 0.6,
    points: [
      () => getArrayWithNoise([-10, 0, -10], 4),
      () => getArrayWithNoise([-2.5, -10, -5], 4),
      () => getArrayWithNoise([2.5, 10, 5], 4),
      () => getArrayWithNoise([10, 0, 10], 4)
    ]
  }),
  ...
  createInstance({
    ...
    duration: 0.4,
    points: [
      () => getArrayWithNoise([0, -20, 0], 1),
      () => getArrayWithNoise([0, 10, 0], 1),
      () => getArrayWithNoise([0, -10, 0], 10),
...
for (let i = 0; i < instances.length; i += 1) {
  ...
  const transitionEnd = transitionBegin + step * 0.8;
  const textEnd = (i + 1) * step;
  uos(
    transitionBegin,
    transitionEnd,
    p => (instances[i].uniforms.time.value = p)
  );
